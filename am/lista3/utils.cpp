...
//returns a permutation 0..(n-1) on indexes 0..(n-1)
...
void find2aprox(vector<int>& permutation, const int cities, const vector<int>& distMatrix) {
    ...
    constructHamiltionCycleThroughDFS(0, adjencyList, visited, permutation, 0); // modyfikacja permutacje -> 2aprox.
}
...
int chooseBestContestant(const vector<int>& draws, const vector<int>& evaluation) {
    int bestOneIdx = draws.at(0);
    int bestEva = evaluation.at(bestOneIdx);
    for(size_t i = 1; i < draws.size(); i++) {
        if(evaluation.at(draws.at(i)) < bestEva) {
            bestOneIdx = draws.at(i);
            bestEva = evaluation.at(draws.at(i));
        }
    }
    return bestOneIdx;
}
...
vector<int> createOffspring(const vector<int>& firstPerm, const vector<int>& secondPerm, int randNum) {
    int n = firstPerm.size();
    vector<int> result;
    result.reserve(n);

    // High-speed lookup tracker
    int maxCity = 0;
    for (int city : firstPerm) {
        if (city > maxCity) maxCity = city;
    }
    vector<bool> inResult(maxCity + 1, false);

    int cut = randNum % n;

    // 1. Copy from first parent up to the cut point
    for (int i = 0; i < cut; ++i) {
        int city = firstPerm[i];
        result.push_back(city);
        inResult[city] = true;
    }

    // 2. Aggressively fill remaining from second parent starting EXACTLY at the cut index
    int p2Pos = cut;
    while (result.size() < n) {
        int city = secondPerm[p2Pos];
        if (!inResult[city]) {
            result.push_back(city);
            inResult[city] = true;
        }
...
void mutate (vector<int>& perm, int idx1, int idx2) {
    ...
    // Use reverse instead of swap!
    reverse(perm.begin() + idx1, perm.begin() + idx2 + 1);
}
...
void drasticalPopulationChange(vector<vector<int>>& currentPopulation, int cities) {
    for(size_t i = 1; i < currentPopulation.size(); i++) {
        random_device rd;
        mt19937 rng(rd());
        int draw = rng() % 100;
        if(draw > 0) {
            currentPopulation.at(i) = generatePermutation(cities);
        }
    }
}
...
int geneticAlgorithm(const int cities, const vector<int>& distMatrix) {
    // auto getDist = [&](int i, int j) { return distMatrix[i * cities + j]; };
    int bestCycle = INT_MAX;

    int numberOfGenerations = 1000 + cities;
    int sizeOfPopulation = 200;
    int numberOfContestantsInTournament = cities / 2;
    vector<vector<int>> currentPopulation;
    vector<int> evaluationOfPopulation;

    vector<int> twoAprox;
    find2aprox(twoAprox, cities, distMatrix);

    random_device rd;
    mt19937 rng(rd());

    //preparing the initial population
    currentPopulation.push_back(twoAprox);
    int cycleDistance = calculatePathDistance(twoAprox, cities,  distMatrix);
    bestCycle = min(cycleDistance, bestCycle);
    evaluationOfPopulation.push_back(cycleDistance);
    for(int i = 1; i < sizeOfPopulation; i++) {
        //generating inital population by applying some swaps on 2aprox
        vector<int> permCopy;
        if(i < sizeOfPopulation /2) {
            permCopy = twoAprox;
            int totalKicks = cities; // Number of random swaps to shake up the tour
            for (int k = 0; k < totalKicks; ++k) {
                int idx1 = 1 + rng() % (cities - 1);
                int idx2 = 1 + rng() % (cities - 1);
                swap(permCopy[idx1], permCopy[idx2]);
            }
        } else {
            permCopy = generatePermutation(cities);
        }
        
        currentPopulation.push_back(permCopy);

        // evaluation of inital population
        cycleDistance = calculatePathDistance(permCopy, cities,  distMatrix);
        bestCycle = min(cycleDistance, bestCycle);
        if(cycleDistance < bestCycle) {
            bestCycle = cycleDistance;
            swap(currentPopulation[0], currentPopulation[i]);
        }
        evaluationOfPopulation.push_back(cycleDistance);
    }
    ...
    while(generationsCounter < numberOfGenerations) {
        ...
        //apocalypse if the result doesn't keep gett ing better
        if(improveCounter > 100) {
            drasticalPopulationChange(currentPopulation, cities);
        }

        //selection (tournament method)
        vector<int> idxOfSelectedParents;
        for(int i = 0; i < sizeOfPopulation; i++) {
            vector<int> draws;
            for(int j = 1; j <= numberOfContestantsInTournament; j++) {
                if(draws.empty()) {
                    int idx = rng() % (sizeOfPopulation);
                    draws.push_back(idx);
                } else {
                    while(true) {
                        int idx = rng() % (sizeOfPopulation);
            ...
            int idx = chooseBestContestant(draws, evaluationOfPopulation);
            idxOfSelectedParents.push_back(idx);
        }

        //crossing
        vector<vector<int>> offsprings;
        for(size_t i = 0; i < idxOfSelectedParents.size(); i += 2) {
            // we draw a random city, that will 'start' the shift to different permutation
            int randNum1 = rng() % (cities);
            // int randNum2 = rng() % (cities);

            int p1 = idxOfSelectedParents.at(i);
            int p2 = idxOfSelectedParents.at(i + 1);

            vector<int> firstOffspring = createOffspring(currentPopulation.at(p1), currentPopulation.at(p2), randNum1);
            vector<int> secondOffspring = createOffspring(currentPopulation.at(p2), currentPopulation.at(p1), randNum1);
            
            offsprings.push_back(firstOffspring);
            offsprings.push_back(secondOffspring);
        }

        // new generation becomes the current population
        currentPopulation = move(offsprings);

        //mutation (1/10 probablity for mutation)
        for(int i = 1; i < sizeOfPopulation; i++) {
            int draw = rng() % 10;
            if(draw <= 9) {
                int idx1 = rng() % (cities);
                int idx2 = rng() % (cities);
                mutate(currentPopulation.at(i), idx1, idx2);
            }
        }

        // evaluation of the new generation
        evaluationOfPopulation.clear();
        for(int i = 0; i < sizeOfPopulation; i++) {
            int cycleDistance = calculatePathDistance(currentPopulation[i], cities, distMatrix);
            if(bestCycle > cycleDistance) {
                improveCounter = 0;
                bestCycle = cycleDistance;
                swap(currentPopulation[0], currentPopulation[i]);
            }
